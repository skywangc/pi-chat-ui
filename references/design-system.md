...
根据已记录版本的上游设计规范整理，作为离线视觉参考；下文组件名称是概念标签，不要求查找外部源码。项目接入步骤以本 skill 为准。尺寸冲突以 component-recipes.md 的实际实现记录优先。本摘录删去了仓库操作指令，保留了大部分英文设计规则。

## Product Character

The workbench chat UI is a desktop-first and web-compatible AI workspace. The interface should feel calm, dense, and operational rather than decorative.
...
- light and dark themes, plus theme variants already supported by the codebase
...
- System
- Light Theme, backed by Light
- Dark Theme, backed by Dark

Default light and dark CSS variables still exist as fallback foundations, but new UI should be validated against Light and Dark as the active light/dark experiences.
...
The workbench chat UI should use restrained depth. Layer primarily through background contrast, borders, and radius before relying on heavy shadows.
...
- Ensure contrast remains safe in light, dark, and theme modes.
